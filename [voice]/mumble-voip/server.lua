...
exports["zero-core"]:object(function(O) Artemis = O end)

function RadioName(id)
	local Player = Artemis.Functions.Player(id)

	if Player then
		local callid = Player.MetaData.callid

		if callid then
			return ("[" .. callid .. "] - " .. Player.PlayerData.firstname .. " " .. Player.PlayerData.lastname)
		else
...
RegisterNetEvent("mumble:callid")
AddEventHandler("mumble:callid", function()
	DebugMsg("Initialised player: " .. source)

	if voiceData[source] then
		voiceData[source]['radioName'] = RadioName(source)
	end
