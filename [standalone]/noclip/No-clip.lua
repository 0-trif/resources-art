        ...
        if IsControlJustPressed(1, config.controls.openKey) then
            Zero.Functions.TriggerCallback('Zero:Server-Admin:IsAdmin', function(result)
                if result then
                    ...
                    TriggerEvent("menu.me.invisible", not noclipActive)
...
RegisterNetEvent('noclip:enable')
AddEventHandler('noclip:enable', function()
    noclipActive = true
end)

RegisterNetEvent('noclip:disable')
AddEventHandler('noclip:disable', function()
    noclipActive = false
end)