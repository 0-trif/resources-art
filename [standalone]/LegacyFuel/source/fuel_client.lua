...
Citizen.CreateThread(function()
	...
	while true do
		...
		if pumpDistance < 2.5 then
			...
			Zero.Functions.GetPlayerData(function(player)
				currentCash = player.Money.cash
			end)
		else
			isNearPump = false
...
AddEventHandler('fuel:refuelFromPump', function(pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		if pumpObject then
			...
			Zero.Functions.DrawText(stringCoords.x, stringCoords.y, stringCoords.z + 1.2, Config.Strings.CancelFuelingPump .. extraString)
			Zero.Functions.DrawText(vehicleCoords.x, vehicleCoords.y, vehicleCoords.z + 0.5, Round(currentFuel, 1) .. "%")
		end
	...
	while true do
		...
		if not isFueling and ((isNearPump and GetEntityHealth(isNearPump) > 0) or (GetSelectedPedWeapon(ped) == 883325847 and not isNearPump)) then
			if IsPedInAnyVehicle(ped) and GetPedInVehicleSeat(GetVehiclePedIsIn(ped), -1) == ped then
				...
				Zero.Functions.DrawText(pumpCoords.x, pumpCoords.y, pumpCoords.z + 1.2, Config.Strings.ExitVehicle)
			else
					...
					if not DoesEntityExist(GetPedInVehicleSeat(vehicle, -1)) then
						...
						if GetVehicleFuelLevel(vehicle) < 95 and canFuel then
							if currentCash > 0 then
								Zero.Functions.DrawText(stringCoords.x, stringCoords.y, stringCoords.z + 1.2, Config.Strings.EToRefuel)
								...
								Zero.Functions.DrawText(stringCoords.x, stringCoords.y, stringCoords.z + 1.2, Config.Strings.NotEnoughCash)
							end
						else
							Zero.Functions.DrawText(stringCoords.x, stringCoords.y, stringCoords.z + 1.2, Config.Strings.FullTank)
						end
					end
				else
					Citizen.Wait(250)
