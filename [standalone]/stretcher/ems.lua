...
RegisterNetEvent("stretcher:client:delete")
AddEventHandler("stretcher:client:delete", function()
    local veh = VehicleInFront()
    local ped = GetPlayerPed(-1)
    local pedCoords = GetEntityCoords(ped)
    local closestObject = GetClosestObjectOfType(pedCoords, 3.0, GetHashKey("prop_ld_binbag_01"), false)
...
-----------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("stretcher:client:spawn")
AddEventHandler("stretcher:client:spawn", function()
    ...
    local str = CreateObject(GetHashKey('prop_ld_binbag_01'), vector3(x,y,z), true)
end)
