...
var last_players:any
var last_chosen_player:any

var Count = function(x:any) {
    let count = -1;
...
onNet("Zero:Client-Bossmenu:SetGrade", (grade:number) => {
    if (last_chosen_player) {
        emitNet('Zero:Server-Bossmenu:SetGrade', last_chosen_player, grade)
    }
})

onNet("Zero:Client-Bossmenu:SetJobGrade", (lastId:number) => {
    if (lastId) {
        var Player = Zero.Functions.GetPlayerData();
        var Job = Player.Job.name;
...
onNet("Zero:Client-Bossmenu:Fire", (lastId:number) => {
    emitNet("Zero:Server-Bossmenu:Fire", lastId)
})

onNet("Zero:Client-Bossmenu:SeePlayer", (Id:number) => {
    var ui = exports['zero-ui'].element();
    var menu:any = {
        [0] : {
            label : "Ontslaan",
...
onNet("Zero:Client-Bossmenu:Employees", (Players:any) => {
    var ui = exports['zero-ui'].element();
    if (last_players) {
        var menu:any = {}
...
onNet("Zero:Client-Bossmenu:Hire", (nothing:any, playerId:number) => {
    emitNet("Zero:Server-Bossmenu:Hire", playerId)
})

onNet("Zero:Client-Bossmenu:DisplayMenu", (Players:any) => {
    var ui = exports['zero-ui'].element();
    if (Players) {
        var menu:any = {}
