        ...
        if not OpenMenu then
            for k,v in pairs(config.tattoo) do
                ...
                if distance <= 4 then
                    ...
                    TriggerEvent("interaction:show", "E - Tattoo menu", function()
                        TriggerEvent("Zero:Client-ClothingV2:Tattoo")
						ped:syncTattoo()
                    end)
                end
            end
...
Citizen.CreateThread(function()
    ...
    SendNUIMessage({
        action = "SetupTattooList",
        list = _,
    })
end)

-- ped -> tattoo functions

function ped:syncTattoo()
	config.skin.tattoo = config.skin.tattoo  ~= nil and config.skin.tattoo or Zero.Functions.GetPlayerData().Skin.skindata.tattoo

    SendNUIMessage({
        action = "SyncSelectedTattoos",
        tats = config.skin.tattoo
    })
end

function ped:tattooApply(dlc, name)
    config.skin.tattoo  = config.skin.tattoo ~= nil and config.skin.tattoo or {}

    ClearPedDecorations(PlayerPedId())

    if (config.skin.tattoo[dlc]) then
        if (config.skin.tattoo[dlc][name]) then
            config.skin.tattoo[dlc][name] = nil
        else
            config.skin.tattoo[dlc][name] = true
        end
    else
        config.skin.tattoo[dlc] = {}
        config.skin.tattoo[dlc][name] = true
    end

    for k,v in pairs(config.skin.tattoo) do
        local dlc = k

        for y, z in pairs(v) do
            ApplyPedOverlay(PlayerPedId(), dlc, y)
        end
    end
...
function ped:loadTats(tats)
	local tats = tats == nil and Zero.Functions.GetPlayerData().Skin.skindata.tattoo or tats

	if tats then
		for dlc,v in pairs(tats)
		do
			for tat , value in pairs(v) 
			do
				ApplyPedOverlay(PlayerPedId(), dlc, tat)
			end
		end

		config.skin.tattoo = tats
	end
end