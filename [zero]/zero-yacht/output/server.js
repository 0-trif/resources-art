...
onNet("Zero:Server-Yacht:Search", (index) => {
    let src = source;
    let Player = Zero.Functions.Player(src);
...
onNet("Zero:Server-Yacht:SearchedSafe", (code) => {
    let src = source;
    let Player = Zero.Functions.Player(src);
    let Inv = Player.Functions.Inventory();
...
Zero.Functions.CreateCallback('yacht:ongoing', function (source, cb) {
    cb(mission.ongoing);
});
Zero.Functions.CreateCallback('yacht:cops', function (source, cb) {
